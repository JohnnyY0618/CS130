...
vec3 Camera::World_Position(const ivec2& pixel_index)
{
    vec3 result;
    //TODO;
    // The world position of a pixel can be calculated by the following formula: p + C_x*u + C_y*v, where p is film position (bottom left corner ofthe screen), u is horizontal vector, v is vertical vector, and C is the vec2 obtained by Cell Center(pixel index);
    vec2 cell_center = (Cell_Center(pixel_index));
    // film_position is p; cell_center[0] is C_x, cell_center[1] is C_y
