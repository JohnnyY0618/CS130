...
void application::draw_line_MPA(int x0, int y0, int x1, int y1, const vec3& linecolor)
{
    ...
    // same as the DDA but MPA use i`````nteger arithmetic and fixed points
    // check if our line is steep or not
    int dy =0; // for store delta y
    int dx =0; // for store delta x
    int xstep = 0; // for store the x increase variable, we use wehn is y steep
    int ystep = 0;  // for store the y increase variable, we use when is x steep
    bool is_y_steep = abs(y1 - y0) > abs(x1 - x0);  // calculate is y steep or not
    if (is_y_steep) {  // if y steep 
        if (y0 > y1) {  // if y0 > y1 swap p0 to p1
            swap(x0, x1);
            swap(y0, y1);
        }
        dx = x1 - x0;
        dy = y1 - y0; 
        if (dx >= 0) {  // if dx is postive our x should increase
            xstep = 1; 
        }
        else { // if dx is negative 
            xstep = -1; // set to -1, our x should be decrease
            dx = -dx;  // make of dx to postive
        }
        int D = 2*dx -dy;  // inialize D using the dinit
        int x = x0;
        for(int y = y0; y < y1; ++y) {
            set_pixel(x, y, linecolor);
            if (D > 0) { // D is pos, we use da
                x = x + xstep;
                D = D + 2*(dx - dy);
            }
            else { // D is neg, we use db
                D = D + 2 * dx;
            }
            
        }
    }
    else { // if x steep, this follow our lab assignment example, check the x0 and x 1
        if (x0 > x1) { // if x0 large than x1 swap p0 to p1
            swap(x0, x1);
            swap(y0, y1);
        }
        dx = x1 - x0;
        dy = y1 - y0;
        if (dy >= 0) { // if dy is postive our y should increase
            ystep = 1;
        }
        else { // if dy is negative our y should decrease
            ystep = -1;  // this would make our y decrese
            dy = -dy; // make dy be postiive
        }
        int D = 2*dy - dx; // inialize D using the dinit
        int y = y0;
        for(int x = x0; x < x1; ++x) {
            set_pixel(x, y, linecolor);
            if (D > 0) {  // D is pos, we use da
